...
BEGIN
    ...
    IF v_existing IS NULL THEN
        -- Authorization guard: only auto-create a license if user has an active
        -- or trialing subscription, or is a superadmin. Prevents any
        -- authenticated user from self-issuing an active license via this
        -- SECURITY DEFINER function (which would otherwise bypass the
        -- superadmin-only INSERT RLS policy on public.licenses).
